...
export default function ClassCard({ classItem }: { classItem: IFacultyClass }) {
            ...
            <div className="mb-4 flex-1 pb-4 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                        <span className="font-medium text-foreground">{classItem.department} • {classItem.batch}</span>
                    </span>
                </div>
            </div>
            <div className="flex justify-center items-center gap-2">
                <Button onClick={() => router.push(`/faculty/classes/attendance/${classItem.id}`)} className="flex-1">Attendance</Button>
                <Button onClick={() => router.push(`/faculty/classes/result/${classItem.id}`)} className="flex-1">Update Result</Button>
            </div>
        </div>
    );
}